import { detectThreats } from '@/lib/security/threat-detection';
...
export async function POST(req: Request) {
  try {
    ...
    const result = await detectThreats(imageData, imageFormat);

    return NextResponse.json(result, { headers: getCorsHeaders() });
  } catch (error) {
    console.error('Error in threat detection API:', error);
    return NextResponse.json(
      {
